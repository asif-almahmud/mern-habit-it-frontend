...
const HabitDetails = ({ habit }) => {
   ...
   const iconStyle =
      "flex justify-center items-center cursor-pointer hover:bg-[#dbf0db] w-[28px] h-[28px] rounded-full active:bg-[#bce4bc]";
   ...
   return (
      <div
         className={` p-4 px-8 border-4 border-gray-400/95 rounded-md ${
            habit.isDone ? "bg-[#f0fff0]" : "bg-white/70"
         } ${habit.isDone ? "" : "hover:bg-[#f9fff9]"} `}
      >
         <h4 className="text-lg font-semibold text-[#72BB9C]">{title}</h4>
         <div className="text-sm text-slate-500">Reps : {reps.join(", ")}</div>
         ...
         <div className="flex gap-3 mt-1">
            ...
            <i className={`fa fa-edit ${iconStyle}`} onClick={handleEdit}></i>
